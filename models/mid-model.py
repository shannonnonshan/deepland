...
class Net(nn.Module):
	def __init__(self):
		...
		self.modulesPDP = modulePDP(in_channels=32, out_channels=32, s=1)
...
class NetBT2(nn.Module):
	def __init__(self, image_size, n_class=10):
		...
		if image_size == 32:
			self.modulesPDP3 = modulePDP(in_channels=64, out_channels=128, s=1)
		else:
		...
		self.modulesPDP8 = modulePDP(in_channels=256, out_channels=256, s=2)
		self.modulesPDP9 = modulePDP(in_channels=256, out_channels=512, s=1)
		self.modulesPDP10 = modulePDP(in_channels=512, out_channels=512, s=2)
		self.conv2 = nn.Conv2d(in_channels=512, out_channels=1024, kernel_size=1, stride=1, padding=0)
		self.avgpool = nn.AdaptiveAvgPool2d(output_size=1)
		self.dropout = nn.Dropout(p=0.2)
		self.fc1 = nn.Linear(1024 * 1 * 1, n_class)
...
def save_plots(history, save_png):
...
def train_single_config(
	...
	model = NetBT2(image_size=image_size, n_class=n_class).to(device)
...
def update_readme_midterm(readme_path, summary_32, summary_224):
	ranked = sorted([summary_32, summary_224], key=lambda x: x["best_test_acc"], reverse=True)
	section = [
		"",
		"## Midterm Report (M1 on Dataset D)",
		"",
		"- Training logs per epoch are saved in outputs_midterm/img_32 and outputs_midterm/img_224",
	...
	for i, item in enumerate(ranked, 1):
		section.append(
			f"{i}. image_size={item['image_size']}, best_test_acc={item['best_test_acc']:.2f}%, best_epoch={item['best_epoch']}"
		)
...
def main():
	...
	parser.add_argument("--output_root", type=str, default="outputs_midterm")
	args = parser.parse_args()

	set_seed(args.seed)
	os.makedirs(args.output_root, exist_ok=True)

	summary_32 = train_single_config(
	...
	summary_224 = train_single_config(
		data_root=args.data_root,
		image_size=224,
		epochs=args.epochs,
	...
	update_readme_midterm("README.md", summary_32, summary_224)

	print("\n=== MIDTERM BEST RESULTS (M1) ===")
	print(f"32x32  -> best test acc: {summary_32['best_test_acc']:.2f}% at epoch {summary_32['best_epoch']}")
	print(f"224x224-> best test acc: {summary_224['best_test_acc']:.2f}% at epoch {summary_224['best_epoch']}")
